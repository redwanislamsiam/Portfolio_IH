...
const ConferenceShortTitle = ({ smSize, e, onSave }) => {
	const {
		register,
		handleSubmit,
		reset,
	...
	useEffect(() => {
		reset({ short_title: e?.short_title });
	}, [e, reset]);
	...
	return (
		<div className=" grid grid-cols-5 items-center gap-5 rounded-xl shadow-lg py-2 hover:shadow-amber-700 outline-none shadow-gray-700 text-sm lg:text-lg mx-5 px-2 md:px-5 my-2 lg:my-5 bg-gray-700/30 transition-all">
			<div className=" col-span-4">
				<label className="label mx-2 lg:mx-5 mb-1 text-white">Short Title</label>
				<input
					type="text"
					...
					id="short_title"
					disabled={!editing}
					defaultValue={e?.short_title}
					{...register("short_title", {
						required: true,
					})}
				/>
			</div>
			{errors.short_title && <span className="label-text-alt text-error">{errors.short_title.message}</span>}
			{editing ?
...
export default ConferenceShortTitle;
