...
const EducationInstitution = ({ smSize, e, onSave }) => {
	const {
		register,
		handleSubmit,
		reset,
	...
	useEffect(() => {
		reset({ institution: e?.institution });
	}, [e, reset]);
	...
	return (
		<div className=" grid grid-cols-5 items-center gap-5 rounded-xl shadow-lg py-2 hover:shadow-amber-700 outline-none shadow-gray-700 text-sm lg:text-lg mx-5 px-2 md:px-5 my-2 lg:my-5 bg-gray-700/30 transition-all">
			<div className=" col-span-4">
				<label className="label mx-2 lg:mx-5 mb-3 text-white">Institution</label>
				<textarea
			...
			{errors.institution && <span className="label-text-alt text-error">{errors.institution.message}</span>}
			{editing ?
