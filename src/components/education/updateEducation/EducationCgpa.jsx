...
const EducationCgpa = ({ smSize, e, onSave }) => {
	const {
		register,
		handleSubmit,
		reset,
	...
	return (
		<div className=" grid grid-cols-5 items-center gap-5 rounded-xl shadow-lg py-2 hover:shadow-amber-700 outline-none shadow-gray-700 text-sm lg:text-lg mx-5 px-2 md:px-5 my-2 lg:my-5 bg-gray-700/30 transition-all">
			<div className=" col-span-4">
				<label className="label mx-2 lg:mx-5 mb-1 text-white">CGPA</label>
                <input
                    type="number"
					...
					{...register("cgpa", {
						...
						validate: (value) => {
							...
							if (!/^\d+(\.\d{1,2})?$/.test(value.toString())) return "Maximum 2 decimal places allowed";
							return true;
						},
					})}
				/>
			</div>
			{errors.cgpa && <span className="label-text-alt text-error">{errors.cgpa.message}</span>}
			{editing ?
