...
const ExpertiseDescritionForm = ({ smSize, e, onSave }) => {
	const {
		register,
		handleSubmit,
		reset,
	...
	useEffect(() => {
		reset({ description: e?.description });
	}, [e, reset]);
	...
	return (
		<div className=" grid grid-cols-5 items-center gap-5 rounded-xl shadow-lg py-2 hover:shadow-amber-700 outline-none shadow-gray-700 text-sm lg:text-lg mx-5 px-2 md:px-5 my-2 lg:my-5 bg-gray-700/30 transition-all">
			<div className=" col-span-4">
				...
				<textarea
					className="input bg-gray-700/30 p-2 text-xs lg:text-sm border-0 w-full text-white outline-none resize-none whitespace-pre-wrap break-words h-30"
					id="description"
					disabled={!editing}
...
export default ExpertiseDescritionForm;